...
print("OPERATOR") >> fname
print("   xor: SIGNAL, SIGNAL -> SIGNAL;") >> fname
print("   and: SIGNAL, SIGNAL -> SIGNAL;") >> fname
print("   neg: SIGNAL -> SIGNAL;") >> fname
...
print("   S: TIME -> TIME;") >> fname
   ...
   print("NOTATION") >> fname
   print("   xor, and, neg, not, or, imply, equal: FUNCTION;") >> fname
   print("   S: FUNCTION;") >> fname
   i=1
   while (i <= numModInpVar) {
      print("   In" i ": FUNCTION;") >> fname
      i++
   }
   i=1
   while (i <= numModOutpVar) {
      print("   Out" i ": FUNCTION;") >> fname
      i++
   }
   i=1
   while (i <= numVar) {
      print("   V" i ": FUNCTION;") >> fname
      i++
   }

print("THEORY") >> fname
print("   xor, and: AC;") >> fname

print("AXIOM") >> fname
print("   [1]           or(P,Q) == xor(and(P,Q),xor(P,Q));") >> fname
print("   [2]            not(P) == xor(P,H);") >> fname
print("   [3]          xor(P,L) == P;") >> fname
print("   [4]          xor(P,P) == L;") >> fname
print("   [5]          and(P,H) == P;") >> fname
print("   [6]          and(P,P) == P;") >> fname
print("   [7]          and(P,L) == L;") >> fname
print("   [8]   and(P,xor(Q,R)) == xor(and(P,Q),and(P,R));") >> fname
print("   [9]        imply(P,Q) == xor(and(P,Q),xor(P,H));") >> fname
print("   [10]       equal(P,Q) == xor(P,xor(Q,H));") >> fname
   ...
   print("    Variables of BLIF_" file ".rdx\n") > vfname
