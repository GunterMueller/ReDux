...
/* If the macro LOGFILE is defined, everything that is printed to
   the (simulated) standard output unit or is read from the standard
   input unit is copied to a logfile on unit 9. */
#define LOGFILE

#define CLOCKSCALING 10
...
/* ##### Begin of CLOCK function. ##### */
#if CLOCKSCALING == 0

/* This is a dummy function is used if no timing facility is available.  */
int CLOCK() { return(0); }
...
#include <sys/times.h>
     ...
     acctime = times(&tbuf);
     return (tbuf.tms_utime * CLOCKSCALING);
}
...
#ifdef LOGFILE
    if ( u == 5 ) {
       if ( f_ptr[9] == NULL )   { /* Open file if not already open. */
          int o;
          o=OPOS; OPOS=0;
          i=IO(9,Open);
          OPOS=o;
       ...
       if ( f_flag[9] != Write_only )    /* Write access? */
          IOERRF(5,"Cannot read from file (read only): ", f_name[9]);

       if (fputs(cbuff,f_ptr[9])==EOF || putc('\n',f_ptr[9])==EOF)
